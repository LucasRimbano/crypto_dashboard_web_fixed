...
1. Un Excel manual con una hoja llamada `Compras`.
2. Un CSV o Excel exportado desde Binance Spot Trade History con columnas como `Date(UTC)`, `Pair`, `Side`, `Price`, `Executed` y `Amount`.
3. Un reporte ya limpiado con una hoja `Datos limpios` y columnas como `Fecha`, `Par`, `Lado`, `Cantidad`, `Precio` y `Total_Cotizacion`.
